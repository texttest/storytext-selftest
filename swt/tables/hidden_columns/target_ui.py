...
tablemenuitem.setText("Show/Hide column")
...
column3 = TableColumn(table, SWT.NONE)
column3.setText("additional")

for i in range(10):
    item = TableItem(table, SWT.NONE)
    item.setText([ "item " + str(i), "edit this value", "Add additional info" ])
...
class ShowColumnListener(Listener):
    def handleEvent(self, event):
        column = table.getColumn(2)
        column.pack() if column.getWidth() == 0 else column.setWidth(0)
        
table.addListener(SWT.MouseDown, TableListener())
tablemenuitem.addListener(SWT.Selection, ShowColumnListener())
