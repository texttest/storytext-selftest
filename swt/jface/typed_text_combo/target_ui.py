...
from org.eclipse.swt.widgets import Display, Shell, Menu, MenuItem, Composite, Label, Text, Combo, Listener
...
label2.setText("City")
text2 = Combo(composite, SWT.NONE)
...
class NameListener(Listener):
    def handleEvent(self, e):
        print "Name is now", repr(e.widget.getText())
        if e.widget.getText() and not e.widget.getText().isdigit():
            deco.hide()
        else:
            deco.show()
