...
from org.eclipse.nebula.widgets.nattable.painter.cell import CheckBoxPainter, CellPainterWrapper, TextPainter
...
from org.eclipse.nebula.widgets.nattable.style import DisplayMode, HorizontalAlignmentEnum, CellStyleUtil, CellStyleAttributes, VerticalAlignmentEnum
...
class InfoPainter(CellPainterWrapper):
    def __init__(self, painter, image):
        CellPainterWrapper.__init__(self, painter)
        self.image = image

    def paintCell(self, cell, gc, bounds, configRegistry):
        CellPainterWrapper.paintCell(self, cell, gc, bounds, configRegistry)
        if self.image is not None:
            imageBounds = self.image.getBounds()
            cellStyle = CellStyleUtil.getCellStyle(cell, configRegistry)
            align = cellStyle.getAttributeValue(CellStyleAttributes.HORIZONTAL_ALIGNMENT)
            dx = 0
            # Try to keep the decorator icon out of the way from the cell contents
            if align is not None and align == HorizontalAlignmentEnum.RIGHT:
            ...
            gc.drawImage(self.image, 
                         bounds.x + dx + CellStyleUtil.getHorizontalAlignmentPadding(align, bounds, imageBounds.width),
                         bounds.y + 2 + CellStyleUtil.getVerticalAlignmentPadding(VerticalAlignmentEnum.TOP, bounds, imageBounds.height))
...
class EditableGridConfiguration(AbstractRegistryConfiguration):
    def configureRegistry(self, configRegistry):
        ...
        image = ImageDescriptor.createFromFile(None, os.path.abspath("showerr_tsk.gif")).createImage()
        configRegistry.registerConfigAttribute(cellattrs.CELL_PAINTER, InfoPainter(TextPainter(), image),
