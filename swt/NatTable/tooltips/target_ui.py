...
from org.eclipse.jface.window import DefaultToolTip, ToolTip
DATE_LABEL = "date_label"

class OurTooltip(DefaultToolTip):
    def __init__(self, natTable):
        DefaultToolTip.__init__(self, natTable, ToolTip.NO_RECREATE, False)
        self.natTable = natTable

    def getToolTipArea(self, event):
        col = self.natTable.getColumnPositionByX(event.x)
        row = self.natTable.getRowPositionByY(event.y)
...
def attachTooltip(table):
    toolTip = OurTooltip(table)
    toolTip.activate()
