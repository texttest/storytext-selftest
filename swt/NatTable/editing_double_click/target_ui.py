...
from org.eclipse.nebula.widgets.nattable.edit.action import MouseEditAction, CellEditDragMode
from org.eclipse.nebula.widgets.nattable.edit.config import DefaultEditBindings
from org.eclipse.nebula.widgets.nattable.edit.editor import TextCellEditor
...
from org.eclipse.nebula.widgets.nattable.grid.layer.config import DefaultGridLayerConfiguration
...
from org.eclipse.nebula.widgets.nattable.ui.matcher import BodyCellEditorMouseEventMatcher
...
def createNatTable(parent):
    ...
    gridLayer = GridLayer(bodyLayer, columnHeaderLayer, rowHeaderLayer, cornerLayer, False)
    gridLayer.addConfiguration(ExtendedGridLayerConfiguration(gridLayer));
...
class MyMouseEventMatcher(BodyCellEditorMouseEventMatcher):
    def matches(self, table, event, labels):
        if event is not None and event.button == 1:
            return BodyCellEditorMouseEventMatcher.matches(self, table, event, labels)
        else:
            return False

class DoubleClickForEditConfiguration(DefaultEditBindings):
    def configureUiBindings(self, uiBindingRegistry):
        mouseEventMatcher = MyMouseEventMatcher(TextCellEditor)
        uiBindingRegistry.registerDoubleClickBinding(mouseEventMatcher, MouseEditAction())
        uiBindingRegistry.registerFirstMouseDragMode(mouseEventMatcher, CellEditDragMode())
        #uiBindingRegistry.registerKeyBinding(new SpecialCharacterKeyEventMatcher(),  new SpecialCharacterKeyEditAction());

class ExtendedGridLayerConfiguration(DefaultGridLayerConfiguration):
    def addEditingUIConfig(self):
        self.addConfiguration(DoubleClickForEditConfiguration())
