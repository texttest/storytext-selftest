...
def createNatTable(parent):
    ...
    gridLayer = nattable.grid.layer.GridLayer(bodyLayer, columnHeaderLayer, rowHeaderLayer, cornerLayer, False)
...
class MyMouseEventMatcher(nattable.ui.matcher.BodyCellEditorMouseEventMatcher):
    def matches(self, table, event, labels):
        if event is not None and event.button == 1:
            return nattable.ui.matcher.BodyCellEditorMouseEventMatcher.matches(self, table, event, labels)
        else:
            return False

class DoubleClickForEditConfiguration(nattable.edit.config.DefaultEditBindings):
    def configureUiBindings(self, uiBindingRegistry):
        mouseEventMatcher = MyMouseEventMatcher(nattable.edit.editor.TextCellEditor)
        uiBindingRegistry.registerDoubleClickBinding(mouseEventMatcher, nattable.edit.action.MouseEditAction())
        uiBindingRegistry.registerFirstMouseDragMode(mouseEventMatcher, nattable.edit.action.CellEditDragMode())
        #uiBindingRegistry.registerKeyBinding(new SpecialCharacterKeyEventMatcher(),  new SpecialCharacterKeyEditAction());

class ExtendedGridLayerConfiguration(nattable.grid.layer.config.DefaultGridLayerConfiguration):
