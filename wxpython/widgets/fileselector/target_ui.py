...
class MyFrame(wx.Frame, GuiCreator):
    ...
    def OnSelectFile(self, event):
        rv = wx.FileSelector(message="Select a File", default_path ="C:\\", default_extension="txt", flags=wx.FD_OPEN)
        if len(rv) == 0:
            self.SetTitle("No file selected")
        else:
            self.SetTitle("File=%s" % rv)
